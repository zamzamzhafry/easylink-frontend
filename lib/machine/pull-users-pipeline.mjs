...
async function importUsersToTbUser(users) {
  ...
  const defaultSn = String(process.env.EASYLINK_DEVICE_SN || '').trim();
  const [snColumnRows] = await pool.query(
    `SELECT 1 AS found
  ...
  const tbUserHasSn = Array.isArray(snColumnRows) && snColumnRows.length > 0;
  if (tbUserHasSn && !defaultSn) {
    throw new Error('tb_user requires SN. Set EASYLINK_DEVICE_SN before importing machine users.');
  }
  let inserted = 0;
  let updated = 0;
      ...
      tbUserHasSn
        ...
        : `INSERT INTO tb_user (pin, nama, pwd, rfid, privilege)
           ...
           ON DUPLICATE KEY UPDATE
             ...
             privilege = VALUES(privilege)`,
      tbUserHasSn
        ? [user.pin, defaultSn, user.nama, defaultPwd, user.rfid || '', Number(user.privilege || 0)]
