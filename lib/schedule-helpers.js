  ...
  const rows = employees
    .map((employee) => {
      const cells = dayHeaders
        .map((dateString) => {
          const shift = getShiftForDate(employee.id, dateString);
          if (!shift) return '<td><span class="shift-pill shift-empty">-</span></td>';
          const icon = shiftIconFor(shift);
          const color = shift.color_hex || '#6B7280';
          const label = escapeHtml(shift.nama_shift || '-');
          return `<td><span class="shift-pill" style="--shift:${color}">${icon} ${label}</span></td>`;
        })
        .join('');
