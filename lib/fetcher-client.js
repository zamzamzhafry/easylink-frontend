...
/**
 * Call the fetcher /fetch endpoint. Returns { ok, fetched, records, error, upstreamStatus }.
 * @param {{sn:string, from:string, to:string, limit?:number}} req
 */
export async function fetchScanlogsFromFetcher({ sn, from, to, limit = 1000 }) {
  ...
  if (!res.ok) {
    ...
    return { ok: false, fetched: 0, records: [], error: `FETCHER_FAILED ${res.status}`, upstreamStatus: res.status, body: text };
  }

  const json = await res.json();
  if (!json || json.ok !== true) {
    return { ok: false, fetched: 0, records: [], error: json?.error || 'FETCHER_BAD_ACK' };
  }

  const records = Array.isArray(json.records) ? json.records : [];
  return { ok: true, fetched: json.fetched ?? records.length, records, error: null };
}
