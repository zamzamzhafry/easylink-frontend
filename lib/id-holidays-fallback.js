...
// Resolve lazily so the dir follows process.cwd() at call time (not import
// time). Lets tests redirect via chdir and avoids capturing a stale cwd if
// the process changes directories after startup.
function dataDir() {
  return join(process.cwd(), 'data', 'holidays');
}
