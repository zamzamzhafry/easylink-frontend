...
export function startOfRange(unit, base = new Date()) {
  ...
  if (unit === 'month') {
    const date = new Date(base);
    const result = new Date(Date.UTC(date.getFullYear(), date.getMonth(), 1));
    return isoDate(result);
  }
  if (unit === 'last') {
    const date = new Date(base);
    const result = new Date(Date.UTC(date.getFullYear(), date.getMonth() - 1, 1));
    return isoDate(result);
  }
  return isoDate(base);
}
...
export function endOfRange(unit, base = new Date()) {
  ...
  if (unit === 'month') {
    const date = new Date(base);
    const result = new Date(Date.UTC(date.getFullYear(), date.getMonth() + 1, 0));
    return isoDate(result);
  }
  if (unit === 'last') {
    const date = new Date(base);
    const result = new Date(Date.UTC(date.getFullYear(), date.getMonth(), 0));
    return isoDate(result);
  }
  return isoDate(base);
}
