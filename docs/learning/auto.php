...
function CreateJSON_ScanLog($conn){
...
function CreateJSON_DataUser($conn){
...
function get_AllLog($conn,$ipServer,$port, $pagingL, $devSN){
	$downloadResult;
	$session=true;
	$delSession=true;	
	...
	while($session){
		...
		if($content->IsSession != $session){
			...
			if (!$handle) { 				
				...
				fwrite ($handle, CreateJSON_ScanLog($conn)); 					
...
function get_AllUser($conn,$ipServer, $port ,$pagingL, $devSN){
	$downloadResult;
	$session=true;
	$delSession=true;
	...
	while($session){
		...
		if($content->IsSession != $session){
				...
				if (!$handle) { 				
					...
					fwrite ($handle, CreateJSON_DataUser($conn)); 					
...
function getDviceFileText($conn){
			...
			for($j=$i; $j<=strlen($t_str); $j++){
				...
				if(strpos(substr($t_str,$j,1),';')!==false){
					...
					if($temp_sn!==''){
						...
						$DownloadStatus=$temp_sn.' : '.get_AllUser($conn,$server_IP,$port,$pagingLimit,$temp_sn).'  '.$DownloadStatus;							
		...
		for($i=0; $i<=strlen($t_str); $i++){
			...
			for($j=$i; $j<=strlen($t_str); $j++){
				...
				if(strpos(substr($t_str,$j,1),';')!==false){
					...
					if($temp_sn!==''){						
						$DownloadStatus=$temp_sn.' : '.get_AllLog($conn,$server_IP,$port,$pagingLimit,$temp_sn).'  '.$DownloadStatus;
					}
